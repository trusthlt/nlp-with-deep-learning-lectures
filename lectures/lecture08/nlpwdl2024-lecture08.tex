...
\begin{frame}{Self-Attention in detailed example}
...
\node[below=0.2cm of X] (inputdsc) {\small Token embeddings (input of previous transformer layer)};
\node[below=0.2cm of matrixh] (convdesc) {\small Projected to query matrix $\bm{Q}$};
\node[below=0.5cm of matrixhtrans] (outdesc) {\small Projected to key matrix $\bm{K}$ and transposed};
\node[below=0.5cm of raw] (outdesc) {\small Scaled dot product $\bm{S}$ (raw associations) \\ (each entry divided by $\sqrt{d_{\text{attn}}}$ )};
...
\node[below=0cm of raw] (outdesc) {\small Scaled dot product (raw associations) \\ (each entry divided by $\sqrt{d_{\text{attn}}}$ )};
