...
\subtitle{Lecture 7 --- BERT as encoder-only transformer}
...
\begin{tikzpicture}[overlay, remember picture] 
	\node at (current page.north east)[ref] {\fullcite{Caruana.1997} \par};
\end{tikzpicture}
...
\begin{tikzpicture}[overlay, remember picture] 
	\node at (current page.north east)[ref] {\fullcite{Sogaard.Goldberg.2016} \par};
\end{tikzpicture}
...
\begin{tikzpicture}[overlay, remember picture] 
	\node at (current page.north east)[ref] {\fullcite{Conneau.et.al.2017.EMNLP} \par};
\end{tikzpicture}
...
\vspace{4em}
The transformer encoder layer is repeated $L$-times (each with \textbf{different} parameters)
...
\begin{frame}{Transformer encoder layer (BERT)}
	...
	\begin{tikzpicture}
		...
		\begin{scope}[thick, black, ->, >=latex]
			...
			\draw (x) -- (sum1);
			\draw (mhattention) edge[out=0,in=270] (sum1) node [right of=mhattention, xshift=0.6cm, yshift=0cm] {$\bm{\tilde{X}}$};
			\draw (sum1) -- (norm1) node [midway, above] {$\bm{\tilde{X}}$};
			\draw (norm1) -- (sum2)  node [near start, above] {$\bm{\tilde{X}}$};
			\draw (norm1) edge[out=0,in=170] (mlp1);
			\draw (mlp1) -- (gelu);
			\draw (gelu) -- (mlp2);
			\draw (mlp2) edge[out=10,in=270] (sum2) node [right of=mlp2, xshift=0.3cm, yshift=0cm] {$\bm{\tilde{X}}$};
			\draw (sum2) -- (norm2) node [midway, above] {$\bm{\tilde{X}}$};
			\draw (norm2) -- (out);
		\end{scope}	
	\end{tikzpicture}
...
\begin{frame}{Multi-head unmasked self-attention}
	...
	\begin{tikzpicture}
		...
		\node (out) [right of=linear, xshift=1cm] {$\bm{\tilde{V}}$};		
...
\begin{frame}{Single unmasked self-attention head}
...
\iffalse
\begin{frame}{Let's explore the attention for a single token first}
...
\begin{frame}{Self-attention in detail --- query, key, scaled dot product}
...
\matrix[mtx, ampersand replacement=\&, nodes={anchor=west}, right=of matrixhtrans] (raw) {
   ...
   S_{[3,1]} \&  \& \ddots \&  \& \vdots \\
   S_{[4,1]} \&  \&  \&  \& \vdots \\
   S_{[5,1]} \&  \cdots \& \cdots  \& \cdots  \& S_{[5,5]} \\
};
...
\foreach \in in {1,2,3,4,5} {
\node[hltr, fill=orange, fit=(matrixhtrans-1-\in)(matrixhtrans-3-\in)] {};
}
...
\node[below=0.2cm of X] (inputdsc) {\small Token embeddings $\bm{X}$ (input or previous transformer layer)};
\node[below=0.2cm of matrixh] (convdesc) {\small $\bm{X}$ projected to query matrix $\bm{Q}$};
\node[below=0.5cm of matrixhtrans] (outdesc) {\small $\bm{X}$ projected to key matrix $\bm{K}$ and transposed};
\node[below=0.5cm of raw, text width=6cm] (outdesc) {\small Scaled dot product $\bm{S}$ (raw associations) \\ (each entry divided by $\sqrt{d_{\text{attn}}}$ )};
\end{scope}
...
\begin{frame}{Self-attention in detail --- softmax over scaled dot product}
...
\matrix[mtx, ampersand replacement=\&, nodes={anchor=west}, right=4cm of raw] (probabilistic) {
   \tilde{S}_{[1,1]} \& \tilde{S}_{[1,2]} \& \tilde{S}_{[1,3]} \& \tilde{S}_{[1,4]} \& \tilde{S}_{[1,5]} \\
   \tilde{S}_{[2,1]} \&  \& \& \& \tilde{S}_{[2,5]}   \\
   \tilde{S}_{[3,1]} \&  \& \ddots \&  \& \tilde{S}_{[3,5]}  \\
   \tilde{S}_{[4,1]} \&  \&  \&  \& \tilde{S}_{[4,5]}  \\
   \tilde{S}_{[5,1]} \&  \cdots \& \cdots  \& \cdots  \& \tilde{S}_{[5,5]} \\
};
...
%\node at ($(matrixh.east)!0.5!(matrixhtrans.west)$) {$\times$};%
\node at ($(raw.north)!0.5!(probabilistic.north)$) {\small Softmax (per row)};%
...
\node[below=0cm of raw, text width=6cm] (outdesc) {\small Scaled dot product (raw associations) \\ (each entry divided by $\sqrt{d_{\text{attn}}}$ )};
\node[below=0cm of probabilistic] (outdesc) {\small Normalized associations};
...
\begin{frame}{Self-Attention in detail --- head output by weighting the value}
...
\foreach \in in {1,2,3,4,5} {
	\node[hltr, fill=red, fit=(selfattention-\in-1)(selfattention-\in-3)] {};
}
\end{scope}
...
\begin{scope}[every node/.style={align=center,text width=3cm}]
%\node[below=0.2cm of X] (inputdsc) {\scriptsize Input tokens};
\node[below=0.2cm of matrixh] (convdesc) {\small $\bm{X}$ projected to value matrix $\bm{V}$};
% \node[below=0.5cm of raw] (outdesc) {\scriptsize Dot product (raw associations)};
\node[below=0cm of probabilistic] (outdesc) {\small Normalized associations};
\node[below=0cm of selfattention] (outdesc) {\small Output $\bm{Y}^h$ of $y$-th self-attention head};
%\node[above=0cm of hidden] (hiddendsc) {\scriptsize Hidden layer};
%\node[above=0cm of output] (outputdsc) {\scriptsize Output};
