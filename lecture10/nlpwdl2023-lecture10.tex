...
\begin{itemize}
\item skipped some important architectural details
\item were unprecise with graphical representation and alogritms
\item did not talk about pre-training and fine-tuning
\end{itemize}
...
\begin{frame}{Transformer encoder (BERT)}
	...
	\begin{tikzpicture}
		...
		\node (out) [right of=finallayer, xshift=1cm] {$\bm{P_{\theta}}$};
...
\begin{frame}{BERT parameters and hyperparameters}
	...
	\begin{minipage}[t][10cm][t]{15cm}
		...
		Hyperparameters: $\ell_{\text{max}}, d_{\text{e}}, d_{\text{mlp}}, d_{\text{f}} \in \mathbb{N}$
...
\begin{frame}{Multi-head unmasked self-attention (BERT)}
	...
	\begin{tikzpicture}
		...
		\begin{scope}[thick, black, ->, >=latex]
			...
			\draw (aheadH) edge[out=0,in=190] (stack) node [right of=aheadH, xshift=0.5cm, yshift=0.6cm] {$\bm{Y^H}$};
...
\begin{frame}{Single unmasked self-attention head (BERT)}
	...
	\begin{tikzpicture}
		...
		\node (matmul) [param, right of=softmax, text centered, text width=1.5cm, xshift=1.5cm] {Matmul $\bm{\tilde{S}}\bm{V}$};
